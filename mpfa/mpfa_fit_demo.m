...
%
%   Pr, N, PPR and variance/mean statistics are estimated per bouton and
%   aggregated over the same fixed, all-pulses-valid MLE bouton list upstream.
%   The held-out PPR target therefore comes directly from the profiles matrix.
...
%   See also MPFA_FIT_TRAIN, MPFA_TRAIN_CORE, MPFA_NPQ_SIMULATOR.
    ...
    for ii = 1:nCond
        ...
        data = struct('pr',M(:,1+c)','N',M(:,7+c)','ppr',M(:,13+c)', ...
    ...
    fprintf(['RMSE is in native units (Pr, sites, ratio). Nmax and Pmax are absolute\n', ...
             ...
             'depression. The held-out PPR target uses the same fixed per-bouton\n', ...
             'MLE-valid list and aggregation as N and Pr.\n\n']);
