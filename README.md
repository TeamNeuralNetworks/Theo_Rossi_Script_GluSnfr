...
For an interactive explanation of the current sequential decomposition workflow and its connection to the biological questions in our study (Rossi, Perrot, Huber, Poulain, Doussau, Valera & Isope — *Bouton‑Specific Diversity of Glutamate Release from Single Axons*), see the publication notebook [`NNLS_Lecture_Demo.ipynb`](docs/nnls_lecture/NNLS_Lecture_Demo.ipynb). It covers kinetic calibration, tri-exponential template variants, sequential event construction, two-pass fraction regularization, prior-tail amplitude correction, and baseline-derived failure thresholds using runnable simulations and publication-style figures.

For a visual catalogue of all registered event-fitting models, their parameters, active bounds, labeled response features, and one-parameter-at-a-time sensitivity, see [`Model_Fitting_Gallery.ipynb`](docs/model_fitting/Model_Fitting_Gallery.ipynb).
