# -*- coding: utf-8 -*-
"""
Created on Wed Sep  4 17:16:34 2019
...
for f in range(len(folders_files)):
    if 'linescan3.xlsx' in folders_files[f]: #ADAPTER nom fichier excel
    ...
    if folders_files[f].startswith('20190801_linescan3'): #ADAPTER nom dossier
    ...
    if 'linescan3_[xy]T_ROI.tif' in folders_files[f]: #ADAPTER nom fichier .tif
        ...
        f_back_start = np.ravel(np.where(distance_vector >= 5))[0] #ADAPTER
        f_back_stop = np.ravel(np.where(distance_vector <= 15))[-1] #ADAPTER
...
#        peaks_indexes = spsi.find_peaks(slim_trace, height=threshold) #Donne valeurs et indices pics > threshold
        indexes = peakutils.indexes(slim_trace, thres=0.45).tolist() #Donne uniquement indices des pics > threshold #ADAPTER
        indexes.remove(684)
...
for i in range(len(indexes)):
    ...
    gs = gridspec.GridSpec(2,5) #ADAPTER donne nombre subplot linescan, ici 10
    ...
    for f in range(len(folders_files)):
        if folders_files[f].startswith('20190801_linescan3'): #ADAPTER nom dossier
            ...
            y2 = np.ravel(np.where(time_vector <= 0.9))[-1]
            curve1 = dF_F0[y1:y2]
...
global_convolved_average = np.mean(LIST_CONVOLVED_dF_F0, axis=0) #ADAPTER PROFILS A MOYENNER
print('Button 3 is not a button')
...
print(df)
with pd.ExcelWriter('D:/Theo.ROSSI/GluSnFR/Virus_AAV.hSynap.SF-iGluSnFR_S72A/AAVDJ.GluSnFR-S72A/AAVDJ.GluSnFR-S72A_Amp_20190801_linescan3.xlsx') as writer:
    df.to_excel(writer, sheet_name='20190801_linescan3', header=False, index=False) #ADAPTER SHEET_NAME
