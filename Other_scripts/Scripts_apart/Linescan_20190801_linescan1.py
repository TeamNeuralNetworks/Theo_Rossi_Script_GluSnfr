# -*- coding: utf-8 -*-
"""
Created on Fri Aug 30 17:15:22 2019
...
for f in range(len(folders_files)):
    if 'coordinates_linescan1.xlsx' in folders_files[f]: #ADAPTER nom fichier excel
    ...
    if folders_files[f].startswith('20190801_linescan1'): #ADAPTER nom dossier
    ...
    if 'linescan1_[xy]T_ROI.tif' in folders_files[f]: #ADAPTER nom fichier .tif
        ...
        f_back_start = np.ravel(np.where(distance_vector >= 199))[0] #ADAPTER
        f_back_stop = np.ravel(np.where(distance_vector <= 210))[-1] #ADAPTER
...
LIST_CONVOLVED_dF_F0_AVERAGE = []
BIG_LIST_AMPS = []

for i in range(len(indexes)):
    LIST_FILES = []
    LIST_FITTED_dF_F0 = []
    LIST_FITTED_dF_F0_AVERAGE = []
    ...
    gs = gridspec.GridSpec(2,10) #ADAPTER donne nombre subplot linescan, ici 10
    ...
    for f in range(len(folders_files)):
        if folders_files[f].startswith('20190801_linescan1'): #ADAPTER nom dossier
                ...
                if '.xlsx' in folders[folder]:
                    ...
                    for file in the_folder:
                        if 'ch_1.tif' in file:
                            ...
                            button = corrected_fluo[POSITIVE_2SIGMA_INDEXES[i] : NEGATIVE_2SIGMA_INDEXES[i], :]
                            ...
                            LIST_dF_F0_AVERAGE.append(dF_F0)
                            ...
                            fitted_dF_F0 = dF_F0[0:-1]-curve_e
                            LIST_FITTED_dF_F0.append(fitted_dF_F0)
                            
                            convolved_dF_F0 = np.convolve(fitted_dF_F0, np.ones((12,))/12, mode='valid') #ADAPTER
            ...
            f0_average = np.mean(average_button[f0_start:f0_stop])
            
            #Determination du dF/F0
            dF_F0_average = (average_button-f0_average)/f0_average
            LIST_dF_F0_AVERAGE.append(dF_F0_average)
        
            #Intervalle [0:0.5] sur les raw data pour la courbe 1
            
            curve1_average = dF_F0_average[y1:y2]
            popt_average, pcov_average = opt.curve_fit(func, time_vector[y1:y2], curve1_average)
...
#            print('a = %s, b = %s, c = %s' %(popt[0], popt[1], popt[2]))
            ...
            curve_e_average = func(time_vector[x1:x2],*popt_average)
            
            #Suppression du photobleaching
            fitted_dF_F0_average = dF_F0_average[0:-1]-curve_e_average
            LIST_FITTED_dF_F0_AVERAGE.append(fitted_dF_F0_average)
    
            convolved_dF_F0_average = np.convolve(fitted_dF_F0_average, np.ones((12,))/12, mode='valid') #ADAPTER
            LIST_CONVOLVED_dF_F0_AVERAGE.append(convolved_dF_F0_average)
#
#            
...
#                print('------')
                ...
                mean_bsl = np.mean(convolved_dF_F0_average[bsl1:bsl2])
                peak_value = np.max(convolved_dF_F0_average[bsl3:bsl4])
...
#            ax2.plot(time_vector[1:-1], fitted_dF_F0, color='k', alpha=0.4, label='Fitted curve')
            ax2.plot(time_vector[12:-1], convolved_dF_F0_average, cm[i], label='Convolved data')
...
##    print(LIST_PEAK_VALUE)       
    ...
    fig, ax = plt.subplots(len(LIST_CONVOLVED_dF_F0), 1, figsize=(8,10), sharex=True, sharey=True, tight_layout=True)
    for l in range(len(LIST_CONVOLVED_dF_F0)):
        ax[l].plot([time_vector[0], time_vector[-1]], [0,0], 'r', linestyle='--', linewidth=2)
#        ax[l].plot([1,1], [-0.5,1], 'r', linestyle='--', linewidth=2)
        ax[l].plot(time_vector[13:-1], LIST_CONVOLVED_dF_F0[l][0:-1], 'k'), ax[-1].set_xlabel('Time (s)', fontsize=15), ax[-1].set_ylabel('dF/F0', fontsize=15), ax[l].grid(True, linestyle='--')
        ax[l].set_title('LINESCAN %s'%(l))
        for stim in stims:
            ax[l].plot([stim,stim],[-0.3,1], 'r', linestyle='--', linewidth=1)

    

fig, ax = plt.subplots(len(LIST_CONVOLVED_dF_F0_AVERAGE), 1, figsize=(15,10), sharex=True, sharey=True, tight_layout=True)
for k in range(len(LIST_CONVOLVED_dF_F0_AVERAGE)):
    ax[k].plot([time_vector[0], time_vector[-1]], [0,0], 'r', linestyle='--', linewidth=2)
    ax[k].plot([1,1], [-0.3,0.5], 'r', linestyle='--', linewidth=2)
    ax[k].plot(time_vector[12:-1], LIST_CONVOLVED_dF_F0_AVERAGE[k], cm[k]), ax[-1].set_xlabel('Time (s)', fontsize=15), ax[-1].set_ylabel('dF/F0', fontsize=15), ax[k].grid(True, linestyle='--')
    ax[k].set_title('BUTTON %s AT INDEX %s'%(k, indexes[k]))

plt.figure(figsize=(15,10))
global_convolved_average = np.mean(LIST_CONVOLVED_dF_F0_AVERAGE, axis=0) #ADAPTER PROFILS A MOYENNER
#print('Buttons 2 and 4 have not been taken into account in the global average')
plt.plot([time_vector[0], time_vector[-1]], [0,0], 'r', linestyle='--', linewidth=2)
plt.plot(time_vector[12:-1], global_convolved_average, 'purple'), plt.xlabel('Time(s)', fontsize=15), plt.ylabel('dF/F0', fontsize=15), plt.grid(True, linestyle='--')
plt.setp(plt.title('CONVOLVED AVERAGE OF ALL BUTTONS', fontsize=20), color='purple')

#plt.savefig(r'D:\Theo.ROSSI\GluSnFR\Virus_AAV.hSynap.SF-iGluSnFR_S72A\AAVDJ.GluSnFR-S72A\2019_05_30\{}.pdf'.format(file))

df = pd.DataFrame(BIG_LIST_AMPS, index=None, columns=None)
print(df)
with pd.ExcelWriter('D:/Theo.ROSSI/GluSnFR/Virus_AAV.hSynap.SF-iGluSnFR_S72A/AAVDJ.GluSnFR-S72A/AAVDJ.GluSnFR-S72A_Amp_20190801_linescan1.xlsx') as writer:
    df.to_excel(writer, sheet_name='20190801_linescan1', header=False, index=False) #ADAPTER SHEET_NAME
