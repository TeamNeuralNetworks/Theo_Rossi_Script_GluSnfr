# -*- coding: utf-8 -*-
"""
Created on Wed Jan 30 16:06:19 2019
...
for f in range(len(folders_files)):
    if '.xlsx' in folders_files[f]: #ADAPTER nom fichier excel
    ...
    if folders_files[f].startswith('20190530_linescan1'): #ADAPTER nom dossier
    ...
    if 'ROI.tif' in folders_files[f]: #ADAPTER nom fichier .tif
        #print('%s this is the ROI file'%folders_files[f])
        ROI_file = plt.imread('%s/%s'%(path, folders_files[f]))
        ROI_file = ROI_file[:,:,0]
        
        for x1, y1 in zip(x_data, y_data):
            increase_x1, increase_y1 = x1+1, y1+1
            X_COORD_1.append(increase_x1)
            Y_COORD_1.append(increase_y1)
            
            decrease_x1, decrease_y1 = x1-2, y1-2 #ADAPTER
            X_COORD_2.append(decrease_x1)
            Y_COORD_2.append(decrease_y1)
        
        for x, y in zip(X_COORD_2, Y_COORD_2):
            pixel = ROI_file[y,x]
            PIXEL_VALUE.append(pixel)
        
        distance_vector = np.arange(0, 100/px_100µm*len(x_data), 100/px_100µm)
        
        f_back_start = np.ravel(np.where(distance_vector >= 80))[0] #ADAPTER
        f_back_stop = np.ravel(np.where(distance_vector <= 100))[-1] #ADAPTER
        ...
        slim_trace = np.convolve(PIXEL_VALUE_WOUT_FBACK, np.ones((5,))/5, mode='valid').tolist() #ADAPTER
        ...
        threshold = 100 #ADAPTER
#        peaks_indexes = spsi.find_peaks(slim_trace, height=threshold) #Donne valeurs et indices pics > threshold
        indexes = peakutils.indexes(slim_trace, thres=0.5).tolist() #Donne uniquement indices des pics > threshold #ADAPTER
...
#        ax2.plot(time_vector_ROI, slim_trace, 'r', label='convolved data'), ax2.set_xlabel(r'Time (ms)')
        ...
        for j in range(len(indexes)):
            interval = slim_trace[indexes[j]-30 : indexes[j]+30]
            LIST_INTERVALS.append(interval)
            x_position = distance_vector2[indexes[j]-30 : indexes[j]+30]
            LIST_X_POSITION.append(x_position)
            plt.plot(distance_vector2[0:60], LIST_INTERVALS[j], cm[j], label='Button %s'%j), plt.grid(axis='y', linestyle='--')
        
        average_interval = np.mean(LIST_INTERVALS, axis=0)
        popt, pcov = opt.curve_fit(gaus, distance_vector2[0:60], average_interval)
        ...
        plt.plot(distance_vector2[0:60], average_interval, color='crimson', linewidth=3, label='Average')
        plt.plot(distance_vector2[0:60], gaus(distance_vector2[0:60], *popt), color='k', linewidth=3, label='Gaussian fit on the average')
        ...
        ax[0].plot(X_COORD_2, Y_COORD_2, 'r')
        ...
        for j in range(len(indexes)):
            ...
            ax[2].plot(LIST_X_POSITION[j], gaus(distance_vector2[0:60], *popt), 'k', linewidth=2)
...
for i in range(len(indexes)):
    ...
    for f in range(len(folders_files)):
        if folders_files[f].startswith('20190530_linescan1'): #ADAPTER nom dossier
            ...
            for folder in range(len(folders)):
                ...
                for file in the_folder:
                    if 'ch_1.tif' in file:
                        ...
                        corrected_fluo = real_data-f_back
                        ...
                        button = corrected_fluo[NEGATIVE_2SIGMA_INDEXES[i] : POSITIVE_2SIGMA_INDEXES[i], :]
            ...
            convolved_dF_F0 = np.convolve(fitted_dF_F0, np.ones((7,))/7, mode='valid') #ADAPTER
            LIST_CONVOLVED_dF_F0.append(convolved_dF_F0)

            #Artefacts de stimulation
            stims = np.arange(0.5,1,0.05) #ADAPTER SELON FREQ STIM
...
#    print(len(LIST_MEAN_BSL))
            
            for j in range(len(LIST_CORRECTED_FLUO)):
                ax1 = fig.add_subplot(gs[0,j])
                ax1.imshow(LIST_CORRECTED_FLUO[j]), ax1.set_xticks(np.arange(0, len(corrected_fluo[1])+200, 200))
                rectangle = patches.Rectangle((0, NEGATIVE_2SIGMA_INDEXES[i]), len(corrected_fluo[1]), (POSITIVE_2SIGMA_INDEXES[i])-(NEGATIVE_2SIGMA_INDEXES[i]), linewidth=0.9, edgecolor='r', facecolor='none')
...
#            ax2.plot(time_vector[1:-1], fitted_dF_F0, color='c', alpha=0.4, label='Fitted curve')
            ax2.plot(time_vector[7:-1], convolved_dF_F0, cm[i], label='Convolved data')
...
for k in range(len(LIST_CONVOLVED_dF_F0)):
    ...
    ax[k].plot(time_vector[7:-1], LIST_CONVOLVED_dF_F0[k], cm[k]), ax[-1].set_xlabel('Time (s)', fontsize=15), ax[-1].set_ylabel('dF/F0', fontsize=15), ax[k].grid(True, linestyle='--')
    ax[k].set_title('BUTTON %s AT INDEX %s'%(k, indexes[k]))

plt.figure(figsize=(15,10))
global_convolved_average = np.mean([LIST_CONVOLVED_dF_F0[0], LIST_CONVOLVED_dF_F0[2], LIST_CONVOLVED_dF_F0[4]], axis=0) #ADAPTER PROFILS A MOYENNER
...
plt.plot(time_vector[7:-1], global_convolved_average, 'c'), plt.xlabel('Time(s)', fontsize=15), plt.ylabel('dF/F0', fontsize=15), plt.grid(True, linestyle='--')
...
print(df)
with pd.ExcelWriter('D:/Theo.ROSSI/GluSnFR/Virus_AAV.hSynap.SF-iGluSnFR_S72A/AAVDJ.GluSnFR-S72A/AAVDJ.GluSnFR-S72A_Amp_20190530_linescan1.xlsx') as writer:
    df.to_excel(writer, sheet_name='20190530_linescan1', header=False, index=False) #ADAPTER SHEET_NAME


#plt.figure()
#
#global_average = np.mean(LIST_dF_F0, axis=0)
#
##Intervalle [0:0.5] sur les raw data pour la courbe 1
#y1 = np.ravel(np.where(time_vector >= 0))[0]
#y2 = np.ravel(np.where(time_vector <= 0.2))[-1]
#curve1 = global_average[y1:y2]
#popt, pcov = opt.curve_fit(func, time_vector[y1:y2], curve1)
#print('Curve 1 coefficients')
#print('a = %s, b = %s, c = %s' %(popt[0], popt[1], popt[2]))
#
##Courbe 1 etendue sur l'intervalle [0:1.9] en utilisant la fonction func
#x1 = np.ravel(np.where(time_vector >= 0))[0]
#x2 = np.ravel(np.where(time_vector <= 1.11))[-1]
#curve_e = func(time_vector[x1:x2],*popt)
#
###Intervalle [0.9:1.9] sur les raw data pour la courbe 2
##Y1 = np.ravel(np.where(time_vector >= 1.02))[0]
##Y2 = np.ravel(np.where(time_vector <= 1.11))[-1]
##curve2 = global_average[Y1:Y2]
##popt_, pcov_ = opt.curve_fit(func, time_vector[Y1:Y2], curve2)
##print('Curve 2 coefficients')
##print('A = %s, B = %s, C = %s' %(popt_[0], popt_[1], popt_[2]))
#
###Courbe 2 etendue sur l'intervalle [0:1.9] en utilisant la fonction func
##X1 = np.ravel(np.where(time_vector >= 0))[0]
##X2 = np.ravel(np.where(time_vector <= 1.11))[-1]
##curve_e2 = func(time_vector[X1:X2],*popt_)
#
#
#
###Moyenne des 2 exp
##mean_curve = (curve_e + curve_e2)/2
##_popt_, _pcov_ = opt.curve_fit(func, time_vector[X1:X2], mean_curve)
##print('Curve 3 coefficients')
##print('A_ = %s, B_ = %s, C_ = %s' %(_popt_[0], _popt_[1], _popt_[2]))
