# -*- coding: utf-8 -*-
"""
Created on Fri Oct 18 15:07:34 2019
...
Amp_excel_file = '20191017_linescan1_20Hz_amp_button3.xlsx'
            ...
            for file in folders:
                if 'ch_1.tif' in file:
                    ...
                    if photobleaching == True:
                        ...
                        y2 = np.ravel(np.where(time_vector <= 0.8))[-1]
                        curve1 = dF_F0[y1:y2]
...
#    print('--------')      
#    print('Button {} at indexes {} to {}.'.format(i, LIST_IDX1[i], LIST_IDX2[i]))
   
    average_button = np.mean(LIST_CONVOLVED_dF_F0, axis=0)
...
#        print('------')
        ...
        if Residual == True:
            bsl1 = np.ravel(np.where(where >= m-0.005))[0]
            bsl2 = np.ravel(np.where(where <= m))[-1]
            mean_bsl = np.mean(average_button[bsl1:bsl2])
            LIST_MEAN_BSL.append(mean_bsl)
            
        bsl3 = np.ravel(np.where(where >= m))[0]
        bsl4 = np.ravel(np.where(where <= n))[-1]
        
        peak_time_index = time_vector[average_button[bsl3:bsl4].argmax()] + m
        peak_index = np.ravel(np.where(time_vector >= time_vector[average_button[bsl3:bsl4].argmax()] + time_vector[bsl3]))[0]
        ...
        for value1, value2 in zip(range(len(LIST_NEG_PEAK_INDEX)), range(len(LIST_POS_PEAK_INDEX))):
            peak_value = np.mean(average_button[LIST_NEG_PEAK_INDEX[value1]:LIST_POS_PEAK_INDEX[value2]])
    ...
    if Residual == True:
        ...
        BIG_LIST_AVERAGE_AMPS.append(LIST_PEAK_VALUE)
    
    
    for trace in LIST_CONVOLVED_dF_F0:
        raw_peak = [np.mean(trace[LIST_NEG_PEAK_INDEX[x1]:LIST_POS_PEAK_INDEX[x2]]) for x1,x2 in zip(range(len(LIST_NEG_PEAK_INDEX)), range(len(LIST_POS_PEAK_INDEX)))]
        LIST_RAW_PEAK.append(raw_peak)
        
        if Residual == True:
            raw_bsl = [np.mean(trace[np.ravel(np.where(where >= pulse-0.005))[0]:np.ravel(np.where(where <= pulse))[-1]]) for pulse in stims]
            LIST_RAW_BSL.append(raw_bsl)
            raw_amps = [raw_peak[s]-raw_bsl[r] for r,s in zip(range(len(raw_bsl)), range(len(raw_peak)))]
            LIST_RAW_AMPS.append(raw_amps)
       
    if Residual == True:    
        BIG_LIST_RAW_AMPS.append(LIST_RAW_AMPS)
    else:
        BIG_LIST_RAW_AMPS.append(LIST_RAW_PEAK)
    
#    print('List mean bsl:{}'.format(LIST_MEAN_BSL))
#    print('List peak value:{}'.format(LIST_PEAK_VALUE))
#print('List average amps:{}'.format(BIG_LIST_AVERAGE_AMPS))
...
#    ax2.plot(time_vector[1:-1], dF_F0[1:-1], 'k', alpha=0.4), ax2.set_ylabel('dF/F0')
    ...
    for z in range(len(LIST_CONVOLVED_dF_F0)):
        ...
        ax2.plot(where, LIST_CONVOLVED_dF_F0[z], 'k', alpha=0.1)
        ax2.plot(where, average_button, cm[i], linewidth=3)
...
#print('Raw amplitudes: {}'.format(BIG_LIST_RAW_AMPS))   

if Dataframe == True:
    df = pd.DataFrame(BIG_LIST_RAW_AMPS[Button], index=None, columns=None)
      
    mean, stdev, sem = df.mean(), df.std(), df.sem()
    
    frame = [df, mean, stdev, sem]
    result = pd.concat(frame)
    print(result)
    
    with pd.ExcelWriter('{}/{}/{}'.format(Path, Big_folder, Amp_excel_file)) as writer:
        result.to_excel(writer, header=False, index=False)
...
#    if photobleaching == True:
#        ax2.plot([time_vector[moving_average], time_vector[-1]], [0,0], 'r', linestyle='--', linewidth=2), ax2.set_xlabel('Time (s)'), ax2.set_ylabel('dF/F0'), ax2.grid(True, linestyle='--')
#        ax2.plot(time_vector[moving_average:-1], convolved_dF_F0, cm[i], linewidth=3)
#    else:
#        ax2.plot([time_vector[moving_average-2], time_vector[-1]], [0,0], 'r', linestyle='--', linewidth=2), ax2.set_xlabel('Time (s)'), ax2.set_ylabel('dF/F0'), ax2.grid(True, linestyle='--')
#        ax2.plot(time_vector[moving_average-2:-1], convolved_dF_F0, cm[i], linewidth=3)
#    
#    
#    for stim in stims:
#        ax2.scatter(stim, 0, c='k', marker='*', linewidths=3)
