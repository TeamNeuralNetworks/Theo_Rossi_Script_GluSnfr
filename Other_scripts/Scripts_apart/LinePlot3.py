# -*- coding: utf-8 -*-
"""
Created on Wed Jan  8 20:26:19 2020
...
Big_folder = '2019_10_29'
Idx_excel_file = '20191029_linescan3_20Hz_idx.xlsx'
Linescan_folder = 'linescan3_20Hz_files'
Amp_excel_file = '20191029_linescan3_20Hz_amp.xlsx'
...
for i in range(len(path2)):
    ...
    for m,n,index in zip(stims, stim_off, range(len(stims))):
        ...
        if Residual == True:
            bsl3 = np.ravel(np.where(where >= m-0.004))[0]
            bsl4 = np.ravel(np.where(where <= m))[-1]
    ...
    if Residual == True:
        for a in range(len(LIST_PEAK_INDEX)):
            if LIST_PEAK_INDEX[a] == LIST_PEAK_INDEX[0]:
                bsl3 = np.ravel(np.where(where >= 0.3))[0]
                bsl4 = np.ravel(np.where(where <= 0.5))[-1]
                mean_bsl = np.mean(convolved_dF_F0[bsl3:bsl4])
                LIST_MEAN_BSL[0] = mean_bsl
...
if Dataframe == True:
    df = pd.DataFrame([BIG_LIST_AVERAGE_AMPS[3], BIG_LIST_AVERAGE_AMPS[4]], index=None, columns=None)
    print(df)
